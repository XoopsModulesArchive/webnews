...
?>
    <rss version="2.0">
        <channel>
            ...
            $nntp->quit();
            ?>
            <link><?php echo construct_url('index.php?group=' . $_SESSION['newsgroup']); ?></link>
            <?php
            echo '<lastBuildDate>' . date('D, d M Y H:i:s O', time()) . '</lastBuildDate>';
            ?>
            <generator>Web-News v.1.5.7 (NNTP to RSS Engine)</generator>
            <image>
                <url>http://web-news.sourceforge.net/images/webnews.gif</url>
                <title>Web-News logo</title>
                <link>
                http://web-news.sourceforge.net</link>
                <width>40</width>
                <height>40</height>
                <description>RSS feed provided by Web-News v.1.5.7 (NNTP to RSS Engine)</description>
            </image>
            <?php
            foreach ($message_summary as $mid => $message) {
                ...
                $link = htmlescape(construct_url('index.php?art_group=' . $_SESSION['newsgroup'] . "&article_id=$mid"));
